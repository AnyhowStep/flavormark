...
export class SmartQuoteParser extends DelimitedInlineParser {
    ...
    public getDelimiterContent (stream : RegexStream, delimiterStartPosition : number, delimiter : number) : string {
    ...
    public tryParse (args : ParseArgs, delimiter : number) : void {
        if (delimiter == C_SINGLEQUOTE) {
